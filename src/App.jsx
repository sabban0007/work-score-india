...
function ScoreBadge({ score }) {
  const color = score >= 70 ? C.green : score >= 40 ? "#c47a1a" : C.red;
  return (
...
// CIBIL/credit-score style gauge — big semicircle dial with color bands
// (Poor / Fair / Good / Excellent) and the score number in the center.
function ScoreGauge({ score }) {
  const bands = [
    { max: 40, color: "#d64545", label: "Poor" },
    { max: 70, color: "#c47a1a", label: "Fair" },
    { max: 90, color: "#4f9e5f", label: "Good" },
    { max: 100, color: "#1f7a34", label: "Excellent" }
  ];
  ...
  const needle = pointOnArc(score);
  let acc = 0;
  const segs = bands.map((b) => {
    const start = acc, end = b.max;
    acc = b.max;
    const p1 = pointOnArc(start), p2 = pointOnArc(end);
    const large = end - start > 50 ? 1 : 0;
    return { d: `M ${p1.x} ${p1.y} A ${r} ${r} 0 ${large} 1 ${p2.x} ${p2.y}`, color: b.color };
  });
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ marginTop: -8, textAlign: "center" }}>
        <span style={{ fontSize: 40, fontWeight: 800, color: band.color, lineHeight: 1 }}>{score}</span>
        <p style={{ fontSize: 12, fontWeight: 700, color: band.color, margin: "2px 0 0", letterSpacing: 1, textTransform: "uppercase" }}>{band.label}</p>
      </div>
    </div>
  );
}
...
const QUICK_FILTER_SKILLS = ["Electrician", "Plumber", "Driver", "Mistri", "Welder", "Helper"];
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
        {QUICK_FILTER_SKILLS.map((s) => (
          <button
            key={s}
            onClick={() => setQuery(query === s ? "" : s)}
            style={{
              fontSize: 12, padding: "6px 12px", borderRadius: 999, cursor: "pointer",
              border: `1px solid ${query === s ? C.text : C.border}`,
              background: query === s ? C.text : "#ffffff",
              color: query === s ? "#ffffff" : C.text
            }}
          >
            {s}
          </button>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 20px", color: C.subtext }}>
          ...
          <p style={{ fontSize: 14, margin: 0 }}>{t.noResults}</p>
        </div>
      ) : (
    ...
    <div style={{ position: "fixed", inset: 0, background: "#000000a0", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 30, padding: 16 }} onClick={onClose}>
      <div style={{ background: C.card, border: `1px solid ${C.border}`, width: "100%", maxWidth: 420, borderRadius: 22, padding: 24, maxHeight: "85vh", overflowY: "auto", boxShadow: "0 10px 30px rgba(0,0,0,0.12)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 4 }}>
          <ScoreGauge score={s.total} />
        </div>
        <p style={{ textAlign: "center", fontSize: 12, color: C.subtext, marginBottom: 14 }}>{t.workScore}</p>
        <Badges w={worker} t={t} />
